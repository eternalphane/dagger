...
func (s *addressSchema) file(
	...
	addr := r.Self().Value
	if matched, err := core.ResolveModuleRef(ctx, addr, &inst); matched {
		return inst, err
	}
...
func (s *addressSchema) workspace(
	...
	return inst, fmt.Errorf("workspace address %q must reference an installed module as %s", addr, workspace.ModuleRefPlaceholder)
}
